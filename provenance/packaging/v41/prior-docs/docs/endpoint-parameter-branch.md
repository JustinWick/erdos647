...
The latest returned run, `erdos647_repo_v39_results_20260908T085147Z_14bd13d1`,
passed **17/19 gates**. The window and truncation gates joined the 14 established
gates and the statement interface. These two new passes audited 21 declarations;
the complete passing reports cover 168 distinct declarations with permitted axioms.
Only the cutoff build failed (three presentation/API applications), and the
prime-mass specialization was blocked behind it.

v40 repairs those three sites in `CutoffParameters.lean` without changing any
existing declaration header, parameter, audit, or accepted mathematical module.
It preserves the already-accepted shared power/log adapter. The remaining first-PR
acceptance is the exact prime-cutoff and prime-mass connection, not a new toolchain
or architecture change. See [v40 changes](changes/v40.md).
...
## v38 proof-source interfaces and acceptance targets
...
For the exact `primeMass`, write lambda=primeMass H y. The final source target is
...
power/log adapter) has a checked-in type example and transitive
axiom report target. Expanded definition checks also pin the coefficient quantifier

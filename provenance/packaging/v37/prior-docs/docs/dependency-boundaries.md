...
The import-only wrapper is now
`research/Erdos647Research/Compat/PNTPlus.lean`. It imports the original upstream
module at its pinned path. `AnalyticInputs.lean` consumes that wrapper; it remains
the sole project proof adapter mentioning the upstream theorem API. There is no
project-owned module under the upstream `PrimeNumberTheoremAnd` root.

Only research module paths and import lines changed in v34. The stable public
imports, declaration names, theorem statements, and proof bodies are unchanged.
